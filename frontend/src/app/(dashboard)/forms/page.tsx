...
import { Plus, Search, Edit2, Link as LinkIcon, FileCheck, BarChart2 } from 'lucide-react';
...
export default function FormsPage() {
  ...
  const filtered = forms.filter(f => 
    ...
    (filter === "All" || (filter === "Published" ? f.is_published : !f.is_published))
  );
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-2">
        <div>
          <h2 className="text-xl font-black uppercase" style={{ fontFamily: "'Archivo Black', sans-serif" }}>Lead Forms</h2>
          <p className="text-sm text-muted-foreground mt-0.5">{forms.length} forms</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/40">
              {["Name", "Status", "Updated", "Actions"].map(h => (
          ...
          <tbody>
            {isLoading && (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-muted-foreground">Loading forms...</td>
              </tr>
            )}
            ...
            {filtered.map(f => (
              <tr key={f.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                <td className="px-4 py-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{f.title || "No Title"}</p>
                </td>
                <td className="px-4 py-3">
                  <Badge className={f.is_published ? "bg-green-100 text-green-700" : "bg-muted text-muted-foreground"}>
                    {f.is_published ? "PUBLISHED" : "DRAFT"}
                  </Badge>
                </td>
                <td {...mono("px-4 py-3 text-xs")}>{format(new Date(f.updated_at || f.created_at), 'MMM dd, yyyy')}</td>
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    <button className="p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground"><Edit2 size={12} /></button>
                    {f.is_published && (
                      <button className="p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground"><LinkIcon size={12} /></button>
                    )}
                    <button className="p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground"><BarChart2 size={12} /></button>
                  </div>
                </td>
              </tr>
            ))}
