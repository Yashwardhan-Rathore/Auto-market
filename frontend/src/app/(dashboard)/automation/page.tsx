...
import { Plus, Search, Edit2, Play, Pause, GitMerge, FileText } from 'lucide-react';
    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">{automations.length} workflows</p>
        </div>
            ...
            {filtered.map(a => (
              <tr key={a.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                <td className="px-4 py-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{a.description || "No description"}</p>
                </td>
                <td className="px-4 py-3"><Badge className={statusColor[a.status] || statusColor.DRAFT}>{a.status}</Badge></td>
                <td {...mono("px-4 py-3 text-xs")}>{format(new Date(a.updated_at || a.created_at), 'MMM dd, yyyy')}</td>
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    <button className="p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground"><Edit2 size={12} /></button>
                    {a.status === 'PUBLISHED' ? (
                      <button className="p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground"><Pause size={12} /></button>
                    ) : (
                      <button className="p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground"><Play size={12} /></button>
                    )}
                    <button className="p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground"><FileText size={12} /></button>
                  </div>
                </td>
              </tr>
            ))}
