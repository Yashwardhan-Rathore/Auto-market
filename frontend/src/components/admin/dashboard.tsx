...
import {
  ...
  Megaphone, Send, Target, TrendingUp,
} from "lucide-react";
import {
...
export function AdminDashboard() {
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          {[1, 2, 3, 4].map(i => <div className="h-36 animate-pulse rounded-2xl bg-white" key={i} />)}
        </div>
  ...
  if (error)
    return (
      <div className="sa-card p-12 text-center">
  ...
  const cards = [
    ...
    },
    {
      ...
      color: "from-orange-500 to-rose-500", glow: "rgba(249,115,22,.30)",
    },
  ];
  ...
  const pulseRows = [
    { label: "Completed campaigns", value: campaigns?.completed ?? 0, icon: CheckCircle2, tone: "bg-emerald-100 text-emerald-600" },
    { label: "Scheduled",           value: campaigns?.scheduled ?? 0, icon: Clock3,       tone: "bg-violet-100 text-violet-600"  },
    { label: "Pending deliveries",  value: delivery?.pending    ?? 0, icon: Mail,         tone: "bg-amber-100 text-amber-600"   },
    { label: "Failed deliveries",   value: delivery?.failed     ?? 0, icon: Activity,     tone: "bg-rose-100 text-rose-600"     },
  ];
    ...
    <div>
      ...
      <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1
            text-[10px] font-extrabold uppercase tracking-[.18em] text-emerald-700">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map((card, i) => (
          <motion.article
            key={card.label}
            variants={reveal} initial="hidden" animate="show" custom={i}
            className="sa-card stat-card group relative overflow-hidden p-5"
          >
            {/* ambient glow blob */}
            <div className="pointer-events-none absolute -right-8 -top-8 h-28 w-28 rounded-full blur-2xl
              transition-all duration-500 group-hover:scale-125 group-hover:opacity-100"
              style={{ background: card.glow, opacity: 0.55 }}
            />
            {/* icon */}
            <div className={`stat-icon grid h-11 w-11 place-items-center rounded-xl
              bg-gradient-to-br ${card.color} text-white shadow-lg`}>
              <card.icon size={20} />
            </div>
            {/* value */}
            <strong className="mt-5 block text-3xl font-black tracking-tight text-slate-950">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.45fr_.75fr]">
        ...
        <motion.section
          ...
          className="sa-card min-h-[390px] p-6"
        >
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-bold text-slate-900">Campaign pipeline</h2>
              <p className="text-xs text-slate-500">Live campaign status distribution</p>
            </div>
          ...
          <div className="mt-5 h-[290px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chart}>
                ...
                <Area type="monotone" dataKey="value" stroke="#2563eb"
                  strokeWidth={3} fill="url(#campaignFill)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </motion.section>
        ...
        >
          <h2 className="font-bold text-slate-900">Performance pulse</h2>
          <p className="text-xs text-slate-500">Key operational signals</p>
          <div className="mt-6 space-y-3">
            {pulseRows.map(item => (
              <div key={item.label}
                className="pulse-row flex items-center gap-4 rounded-xl
                  border border-slate-100 bg-slate-50/70 p-4">
                <div className={`pulse-icon grid h-10 w-10 place-items-center rounded-xl ${item.tone}`}>
                  <item.icon size={19} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-slate-500">{item.label}</p>
                  <strong className="text-xl text-slate-900">{compact(item.value)}</strong>
                </div>
              </div>
            ))}
          </div>
