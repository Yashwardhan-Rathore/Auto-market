...
export function UserContentStudio(){
  ...
  const draftsQuery=useQuery({queryKey:["user-content-drafts"],queryFn:async()=>{
  ...
  const generate=useMutation({mutationFn:async()=>{const created=(await apiClient.post<ContentDraftData>("/api/content/content-drafts/",{original_prompt:prompt.trim(),platforms:selected},{timeout:60000})).data;setDraftId(created.id);setCurrent(created);try{await apiClient.patch(`/api/content/content-drafts/${created.id}/`,{enhanced_prompt:prompt.trim()},{timeout:60000});await apiClient.post(`/api/content/content-drafts/${created.id}/regenerate/`,{reason:"Initial content generation",generate_images:true,generate_captions:includeCaption},{timeout:240000});return(await apiClient.get<ContentDraftData>(`/api/content/content-drafts/${created.id}/`,{timeout:60000})).data}catch(error){try{setCurrent((await apiClient.get<ContentDraftData>(`/api/content/content-drafts/${created.id}/`,{timeout:60000})).data)}catch{}throw error}},onSuccess:draft=>{setCurrent(draft);toast.success("Prompt-based content and image generated");void client.invalidateQueries({queryKey:["user-content-history"]});void client.invalidateQueries({queryKey:["user-content-drafts"]});},onError:error=>toast.error(parseApiError(error))});
  ...
  if(current)return <ContentPreviewCustomize draft={current} generating={generate.isPending} onChange={setCurrent} onBack={()=>setCurrent(null)} onGenerateNew={()=>{setCurrent(null);setPrompt("");setDraftId(null)}}/>;
  return <div>
